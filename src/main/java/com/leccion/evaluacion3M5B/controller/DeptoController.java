...
import com.leccion.evaluacion3M5B.service.DeptoService;
...
public class DeptoController {
    ...
    public ResponseEntity<Depto> actualizarDepto(@PathVariable Long id, @RequestBody Depto p) {
        Depto deptoEcontrada = deptoService.findById(id);
        if (deptoEcontrada == null) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        } else {
            try {
                deptoEcontrada.setNombre(p.getNombre());
                deptoEcontrada.setDirector(p.getDirector());
                deptoEcontrada.setDescripcion(p.getDescripcion());
