...
import com.leccion.evaluacion3M5B.service.ProfesorService;
...
public class ProfesorController {
    ...
    public ResponseEntity<Profesor> actualizarProfesor(@PathVariable Long id, @RequestBody Profesor p) {
        Profesor cursorEncontrado = profesorService.findById(id);
        if (cursorEncontrado == null) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        } else {
            try {
                cursorEncontrado.setNombre(p.getNombre());
                cursorEncontrado.setDireccion(p.getDireccion());
                cursorEncontrado.setTelefono(p.getTelefono());
                cursorEncontrado.setDepto(p.getDepto());
                cursorEncontrado.setListaCursos(p.getListaCursos());
